...
// Type definitions for each criteria
...
// Generic compatibility data type
...
// Compatibility matrix based on research
...
export default function WalletMatrix() {
  ...
  const handleCellClick = (criteriaKey: string, typeKey: string) => {
    if (mode === 'description') {
      ...
      // Compatibility mode - only allow selection of compatible types
      const isCompatible = isTypeCompatible(criteriaKey, typeKey);
      if (!isCompatible) {
        return; // Do nothing if type is incompatible
      }
  ...
  const handleScenarioSelect = (scenario: string) => {
    ...
    if (scenarioConfig) {
      ...
      // Scroll to the top of the matrix (just below the fixed header)
      setTimeout(() => {
  ...
  // Helper function to safely access compatibility matrix
  ...
  const getCompatibleTypes = (): Set<string> => {
    ...
    Object.entries(selectedTypes).forEach(([criteriaKey, typeKey]) => {
      ...
      if (compatibilityData) {
        ...
        // Add compatible types from other criteria
    ...
    // Find intersection of all compatible sets
              ...
              return (
                <tr key={criteriaKey}>
                  ...
                  {typeKeys.map(typeKey => {
                    ...
                    // Different hover styles for selected vs non-selected cells
